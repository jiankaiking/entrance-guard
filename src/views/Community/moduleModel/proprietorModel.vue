<template>
  <el-dialog title="业主信息" center :visible.sync="dialogTableVisible">
    <div class="menuInfo">
      <el-form ref="form" :model="modelFromdata" :rules="rules" label-width="120px">
        ...
        <el-form-item label="住宅楼名称" prop="buildId">
          <el-select v-model="modelFromdata.buildId" placeholder="请选择住宅楼" @change="changeBuilding">
        ...
        <el-form-item label="单元名称" prop="unitId">
          <el-select v-model="modelFromdata.unitId" placeholder="请选择单元" @change="changeUnit">
            <el-option v-for="item in unitArr" :key="item.id" :label="item.value" :value="item.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="房间号" prop="ownerRoomNo">
          <!--          <el-input v-model="modelFromdata.ownerRoomNo" placeholder="请输入房间号" />-->
          <el-select v-model="modelFromdata.ownerRoomNo" placeholder="请选择单元" @change="changeRoom">
            <el-option v-for="item in roomArr" :key="item.id" :label="item.value" :value="item.id + '-' + item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="住户类型" prop="ownerType">
          <el-select v-model="modelFromdata.ownerType" placeholder="请选择小区名称">
            <el-option v-for="item in ownerTypeArr" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="姓名" prop="ownerName">
          <el-input v-model="modelFromdata.ownerName" placeholder="请输入联系人" />
        </el-form-item>
        <el-form-item label="联系电话" prop="ownerPhone">
          <el-input v-model="modelFromdata.ownerPhone" placeholder="请输入联系电话" />
        </el-form-item>
        ...
        <el-form-item label="证件号码">
          <el-input v-model="modelFromdata.ownerIdNo" placeholder="请输入证件号码" />
        </el-form-item>
...
export default {
  ...
  methods: {
    changeComm(communityId) {
      ...
      this.modelFromdata.roomId = ''
      this.modelFromdata.ownerRoomNo = ''
    ...
    changeRoom(e) {
      if (e) {
        const selectValue = e.split('-')
        this.modelFromdata.roomId = +selectValue[0]
        this.modelFromdata.ownerRoomNo = selectValue[1]
      } else {
        this.modelFromdata.roomId = ''
        this.modelFromdata.ownerRoomNo = ''
      }
    ...
    getRoomArr(unitId) {
      const uniId = unitId || this.modelFromdata.unitId
      httpRequest('/property-manage/community/room/info/list', 'GET', { unitId: uniId })
        .then(res => {
